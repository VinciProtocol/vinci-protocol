...
import { getUserData, getUserNFTVaultData, getReserveData } from './helpers/utils/helpers';

const chai = require('chai');
const { expect } = chai;

makeSuite('LendingPool liquidation - liquidator receiving nToken', (testEnv) => {
  const {
  ...
  it('Deposits CRYPTOPANDA, borrows DAI/Check liquidation fails because health factor is above 1', async () => {
    ...
    await expect(
      pool.nftLiquidationCall(nft.address, dai.address, borrower.address, ['1'], ['1'], true)
  ...
  it('Liquidates the borrow', async () => {
    const { pool, dai, nft, nNFT, aDai, users, oracle, helpersContract, deployer } = testEnv;
    const borrower = users[1];
    ...
    const userNFTVaultDataBefore = await helpersContract.getUserNFTVaultData(nft.address, borrower.address);

    const tx = await pool.nftLiquidationCall(
      nft.address,
      dai.address,
      borrower.address,
      userNFTVaultDataBefore.tokenIds,
      userNFTVaultDataBefore.amounts,
      true
    );
    ...
    const collateralPrice = (await oracle.getAssetPrice(nft.address)).toString();
    ...
    let expectedCollateralLiquidated = new BigNumber(principalPrice)
      .times(new BigNumber(1).times(105))
      .div(new BigNumber(collateralPrice).times(new BigNumber(10).pow(principalDecimals)))
      .decimalPlaces(0, BigNumber.ROUND_DOWN);
    if(expectedCollateralLiquidated.eq(0)) {
      expectedCollateralLiquidated = new BigNumber(1);
    }

    const amountToLiquidate = expectedCollateralLiquidated.times(new BigNumber(collateralPrice));
