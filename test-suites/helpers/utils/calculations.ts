...
interface Configuration {
  reservesParams: iVinciPoolAssets<IReserveParams>;
}
...
export const calcExpectedUserDataAfterDeposit = (
  ...
  expectedUserData.scaledVTokenBalance = calcExpectedScaledVTokenBalance(
    userDataBeforeAction,
    reserveDataAfterAction.liquidityIndex,
    new BigNumber(amountDeposited),
    new BigNumber(0)
  );
  expectedUserData.currentVTokenBalance = calcExpectedVTokenBalance(
    reserveDataBeforeAction,
    userDataBeforeAction,
    txTimestamp
  ).plus(amountDeposited);

  if (userDataBeforeAction.currentVTokenBalance.eq(0)) {
...
export const calcExpectedUserDataAfterWithdraw = (
  ...
  const vTokenBalance = calcExpectedVTokenBalance(
    reserveDataBeforeAction,
    userDataBeforeAction,
    txTimestamp
  );
  ...
  if (amountWithdrawn == MAX_UINT_AMOUNT) {
    amountWithdrawn = vTokenBalance.toFixed(0);
  }
  ...
  expectedUserData.currentVTokenBalance = vTokenBalance.minus(amountWithdrawn);
  ...
  if (userDataBeforeAction.currentVTokenBalance.eq(0)) {
    ...
    if (expectedUserData.currentVTokenBalance.eq(0)) {
...
export const calcExpectedReserveDataAfterWithdraw = (
  ...
  if (amountWithdrawn == MAX_UINT_AMOUNT) {
    amountWithdrawn = calcExpectedVTokenBalance(
      reserveDataBeforeAction,
      userDataBeforeAction,
      txTimestamp
    ).toFixed();
...
export const calcExpectedUserDataAfterBorrow = (
  ...
  expectedUserData.scaledVTokenBalance = userDataBeforeAction.scaledVTokenBalance;
...
const calcExpectedScaledVTokenBalance = (
  userDataBeforeAction: UserReserveData,
  index: BigNumber,
  amountAdded: BigNumber,
  amountTaken: BigNumber
) => {
  return userDataBeforeAction.scaledVTokenBalance
    .plus(amountAdded.rayDiv(index))
    .minus(amountTaken.rayDiv(index));
};

export const calcExpectedVTokenBalance = (
  ...
  const { scaledVTokenBalance: scaledBalanceBeforeAction } = userData;
